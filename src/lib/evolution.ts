...
export async function sendWhatsAppMessage(toPhone: string, text: string, tenantId?: string) {
  // Format phone to numeric string with country code (e.g. 519XXXXXXXX)
  const cleanPhone = toPhone.replace(/\D/g, '');
  const instanceName = tenantId ? `gym_tenant_${tenantId.replace(/-/g, '_')}` : 'default';
